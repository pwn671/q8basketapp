...
export default function CartPage() {
    const { state, dispatch: cartDispatch, itemsCount, subtotal, discount, totalPrice } = useCart();
    ...
    useEffect(() => {
        ...
        fetchShippingQuote();
    }, [state.appliedCoupon, selectedLocationId, selectedAddress, subtotal, discount]);
    ...
    const onPlaceOrder = async (amount) => {
        ...
        if (!shippingQuote) {
            alert('Please wait for shipping charges to load or select address again.');
            return;
        }
    ...
    const handleAddressSelect = async (locationId) => {
        ...
        try {
            ...
            if (addressResult.success) {
                foundAddress = addressResult.data.find(addr => addr.location_id === locationId);
                if (!foundAddress) {
    ...
    const placeOrder = async (locationId) => {
        try {
            ...
            setOrderResult({
                success: false,
                message: error.message || 'Failed to place order. Please check your connection and try again.'
            });
        ...
        <div className={styles.pageWrapper}>
            <div className={styles.pageInner}>
                ...
                <div className={styles.content}>
                        ...
                        {cartItems.length === 0 ? (
                            ...
                            cartItems.map((it) => (
                                <article key={it.id} className={styles.cartItem}>
                                    ...
                                    {/* Center: Info */}
                                    <div className={styles.itemInfo} onClick={() => onOpenProduct(it.id)}>
                                        <img
                                            src={it.image}
                                            alt={it.title}
                                    ...
                                    <div className={styles.itemActions}>
                                        {it.quantity > 0 ? (
                                            <div className={styles.qtyControl}>
                                                <button className={styles.qtyBtn} onClick={() => onDec(it.id)}>-</button>
                                                <span className={styles.qtyCount}>{it.quantity}</span>
                                                <button className={styles.qtyBtn} onClick={() => onInc(it)}>+</button>
                                            </div>
                                        ) : (
                                            <button className={styles.addBtn} onClick={() => onInc(it)}>Add</button>
                                        )}
                                        <div className={styles.itemTotal}>{formatCurrency(it.quantity * it.price)}</div>
                                    </div>
                                </article>
                    ...
                    <section className={styles.couponCard} onClick={() => navigate("/coupon")}>
                        ...
                        <section className={styles.billCard}>
                            ...
                            <div className={styles.billRow}>
                                <div className={styles.billLeft}>
                                    <img src="/icons/notepad.svg" alt="Tag" className={styles.billIcon} />
                            ...
                            <div className={styles.billRow}>
                                <div className={styles.billLeft}>
                                    <img src="/icons/delivery-truck.svg" alt="Delivery" className={styles.billIcon} />
                                    <span>Delivery charge</span>
                                </div>
                                <span style={{ color: !selectedAddress ? '#d32f2f' : shippingQuote ? '#111' : '#ff9800' }}>
                                    {shippingQuote
                                        ? (shippingQuote.shipping === 0 || shippingQuote.shipping === null)
                                            ? 'Free shipping'
                                            : formatCurrency(shippingQuote.shipping || DEFAULT_SHIPPING_CHARGE)
                                        : selectedAddress
                            ...
                            {(shippingQuote?.coupon || state.appliedCoupon) && (shippingQuote?.coupon?.discount || discount) > 0 && (
                                <div className={styles.billRow} style={{ color: '#22c55e' }}>
                                    <div className={styles.billLeft}>
                                        <img src="/icons/discount.svg" alt="Discount" className={styles.billIcon} />
                                        <span>Coupon Discount ({shippingQuote?.coupon?.code || state.appliedCoupon?.code})</span>
                                    </div>
                                    <span>-{formatCurrency(shippingQuote?.coupon?.discount || discount)}</span>
                                </div>
                            )}
                ...
                <div className={styles.locationBar}>
                    <div className={styles.locationLeft}>
                        ...
                        <div className={styles.locText}>
                            ...
                            <div className={styles.locAddress} title={
                                selectedAddress
                    ...
                    <button className={styles.changeBtn} onClick={() => setShowAddress(true)}>Change</button>
                </div>
                ...
                <div className={styles.paymentBar}>
                    ...
                    >
                        ...
                        <div className={styles.placeText}>
                            {selectedAddress && !shippingQuote ? 'Loading Charges...' : 'Place Order'}
                        </div>
                    </button>
                </div>
                <Address
                            ...
                            <div className={styles.confirmDetails}>
                                <div className={styles.confirmRow}>
                                    ...
                                    <span className={styles.confirmValue}>
                                        ...
                                        {selectedAddress?.apartment_building && `, ${selectedAddress.apartment_building}`}
                                    </span>
                                </div>

                                <div className={styles.confirmRow}>
                                    <span className={styles.confirmLabel}>Payment Method:</span>
                                    <span className={styles.confirmValue}>{state.paymentMethod}</span>
                                </div>
                                ...
                                <div className={styles.confirmRow}>
                                    ...
                                    <span className={styles.confirmValue}>
                                        {cartItems.length === 0
                                            ...
                                            : shippingQuote?.payable
                                                ? formatCurrency(shippingQuote.payable)
                                                : formatCurrency(total - discount + DEFAULT_SHIPPING_CHARGE)}
                                    </span>
                                </div>
                            </div>
                ...
                {orderResult && (
                    <div className={styles.orderResultOverlay}>
                        <div className={`${styles.orderResultPopup} ${orderResult.success ? styles.success : styles.failure}`}>
